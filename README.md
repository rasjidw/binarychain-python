...
Binary Chains are a simple but flexible binary container format, primarily meant for sending blocks of binary data over
streaming interfaces like sockets. Similar to NetStrings, but for binary data.


## Binary Chain Format

A binary chain consists of a prefix, which is an ascii string (strictly bytes of value 0x00 to 0x7F) plus zero or more
binary parts.

Each binary part starts with a SOP byte (0x80 to 0x88), the binary part length (big endian encoded), and then the actual
data.
